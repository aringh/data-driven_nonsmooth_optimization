...
output_filename = (save_path + 'Output_' + file_name + '_' +
                   time_str + '.txt')
sys.stdout = Logger(output_filename)  # Creates logger that writes to f
...
with tf.name_scope('variable_definitions'):
    ...
    std = 1e-8
    par_two_init = np.log(1/(np.sqrt(2) - 1))

    par_one = tf.Variable(tf.constant(0, dtype=tf.float32) +
                          tf.truncated_normal([1], stddev=std),
                          name='par_one')
    par_two = tf.Variable(tf.constant(par_two_init, dtype=tf.float32) +
                          tf.truncated_normal([1], stddev=std),
                          name='par_two')
...
for i in range(0, maximum_steps):
    ...
    if i%100 == 99:
        ...
        (sigma_res, tau_res, theta_res, par_one_res, par_two_res,
         par_three_res) = sess.run([sigma, tau, theta, par_one, par_two,
                                    par_three])
        print('sigma={}, tau={}, theta={}, sigma*tau*||L||^2={}'.format(
                sigma_res, tau_res, theta_res,
                sigma_res*tau_res))

        print('par_one={}, par_two={}, par_three={}'.format(
                par_one_res, par_two_res, par_three_res))
