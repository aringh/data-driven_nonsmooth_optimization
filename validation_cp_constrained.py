...
with tf.name_scope('variable_definitions'):
    par_one = tf.Variable(tf.constant(17.3383007, dtype=tf.float32),
                          name='par_one')
    par_two = tf.Variable(tf.constant(17.35003471, dtype=tf.float32),
                          name='par_two')
    par_three = tf.Variable(tf.constant(0.41985849, dtype=tf.float32),
                            name='par_three')
...
for imax_val in n_iter:
    ...
    #                saveto=(save_path + 'cp_const_iter_{}').format(imax_val))

    fig = plt.figure()
    plt.imshow(np.rot90(x_rec_odl.asarray()), cmap='bone', clim=[0, 2.33])
    plt.xticks([])
    plt.yticks([])
    fig.savefig((save_path + 'cp_const_iter_{}').format(imax_val),
    ...
    plt.xticks([])
    plt.yticks([])
    fig.savefig((save_path + 'cp_const_iter_{}_windowed').format(imax_val),
                transparent=True, bbox_inches='tight', pad_inches=0)
