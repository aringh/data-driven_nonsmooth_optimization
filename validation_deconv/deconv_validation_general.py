...
def matmul(x, v):
    return tf.map_fn(lambda y: _helper_matmul(y, v), x)
    # return tf.nn.conv2d(x, v[None, None, ...], strides=[1, 1, 1, 1], padding='SAME')


def _helper_matmul(x, v):
    to_return_list = []

    for i in range(x.shape[2]):
        tmp = tf.zeros((x.shape[0], x.shape[1]))
        for j in range(x.shape[2]):
            tmp = tf.add(tmp, x[:, :, j] * v[j, i])
        to_return_list.append(tmp)

    to_return = tf.stack(to_return_list, axis=2)
    return to_return
