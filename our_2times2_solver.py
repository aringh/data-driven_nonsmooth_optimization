"""Training parameters in proposed solver"""
...
with tf.name_scope('variable_definitions'):
    ...
    par_three = tf.Variable(tf.constant(0, dtype=tf.float32) +
                            tf.truncated_normal([1], stddev=std),
                            name='par_three')
    par_four = tf.Variable(tf.constant(0, dtype=tf.float32) +
                            tf.truncated_normal([1], stddev=std),
                            name='par_four')
...
for i in range(0, maximum_steps):
    ...
    if i%100 == 99:
        ...
        print('sigma={}, tau={}, c21={}, a21={}, sigma*tau*||L||^2={}'.format(
                sigma_res, tau_res, c21_res, a21_res,
                sigma_res*tau_res))

        print('par_one={}, par_two={}, par_three={}, par_four={}'.format(
                par_one_res, par_two_res, par_three_res, par_four_res))
