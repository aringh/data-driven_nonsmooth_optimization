...
fig = plt.figure()
plt.imshow(np.rot90(np.squeeze(x_true_arr_validate)), cmap='bone', clim=[0, 2.33])
plt.xticks([])
plt.yticks([])
fig.savefig(save_path + 'phantom', transparent=True,
            bbox_inches='tight', pad_inches=0)

fig = plt.figure()
plt.imshow(np.rot90(np.squeeze(x_true_arr_validate)), cmap='bone', clim=[0.8, 1.2])
plt.xticks([])
plt.yticks([])
fig.savefig(save_path + 'phantom_windowed', transparent=True,
            bbox_inches='tight', pad_inches=0)
...
for imax_val in n_iter:
    ...
    plt.xticks([])
    plt.yticks([])
    fig.savefig((save_path + 'sidky_iter_{}_windowed').format(imax_val),
                transparent=True, bbox_inches='tight', pad_inches=0)
